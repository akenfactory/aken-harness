...
The launch-token exchange this note describes no longer exists: [admin-login credential gate](../feature/2026-09-07-admin-login-credential-gate.md) replaces it with a mandatory email+password sign-in as `dsh web`'s only way in. This note stays active for the parts of its decision the replacement kept unchanged — one uniform browser session for the complete Host API, the persistent signed-cookie shape and its authority binding, the owner-scoped signing-secret record and its revocation-by-deletion model, and the rejected alternatives (TCP peer address, a method-specific privileged list, a durable bearer token) — while the credential that mints the cookie changed.
