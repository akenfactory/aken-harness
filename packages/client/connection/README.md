...
Every Host RPC method and WebSocket stream requires one browser session; there is no method-specific loopback tier, and there is exactly one way to obtain a session: signing in with the admin email and password configured through `ADMIN_EMAIL`/`ADMIN_PASSWORD`. `requireAdminCredentials` (`src/admin-login.ts`) reads both from the process environment at plugin boot; either missing fails the load loudly — `dsh web` has no other way in, so a half-configured credential must not silently boot anyway. The raw password is hashed (`scrypt`, a fresh random salt) once at boot and discarded; only the derived hash and an email digest are kept in memory for the process lifetime, never persisted or logged.

`frontend-static` delegates root and index requests to `ctx.connection.authorizeIndex`: an unauthenticated `GET /` serves a self-contained, script-free HTML sign-in `<form method="POST">` (`src/login-form.ts`) instead of the plain-text 401; every other unauthenticated request gets the plain-text 401. The form posts to `POST /login` (`src/login-route.ts`), which verifies the submission — constant-time comparison of both the email and the password (`verifyAdminLogin`), so a wrong email costs exactly as much CPU time as a wrong password and neither reveals which one was wrong — and mints an authority-bound signed cookie on success. `src/login-throttle.ts` applies a fixed per-source exponential backoff (500ms doubling, capped at 30s) to failed attempts ahead of running the password hash; the curve is a security invariant, not configurable. A missing, expired, malformed, or wrong-authority cookie returns 401 before RPC dispatch. Static assets remain public. The HTTP carrier accepts no Authorization-header token and no query-string credential of any kind.

The cookie signing secret is the owner-scoped `client-connection/browser-session` grant record in `ctx.credentials`. The local provider persists it in `$DSH_HOME/.credentials.yaml`; `BrowserAuth` loads or creates the record during Connection activation and retains the secret in memory, so request authentication is synchronous. Deleting or replacing the record takes effect on the next Connection activation. Cookies carry an absolute issue/expiry interval, defaulting to 30 days through `cookieMaxAgeDays`, and bind the normalized hostname plus port in both their deterministic name and signed payload. They are host-only, `Path=/`, `HttpOnly`, and `SameSite=Strict`; they deliberately omit `Secure` because the shipped server uses loopback HTTP. The admin credential itself is not persisted anywhere — it is supplied fresh through the environment on every launch — so it has no revocation state of its own; deleting the signing-secret record and restarting revokes every outstanding session.

Before authentication, every request still passes `src/api-request-trust.ts`. Its `Host` must be loopback or match a `trustedHosts` entry: exact on `host:port`, any port on port-less entries, both sides WHATWG-normalized. An attached `Origin` must equal that Host and `sec-fetch-site: cross-site` is refused. Malformed configured authorities fail plugin load. These checks defend DNS rebinding and cross-site browser requests; they never establish identity. A failed Host/Origin check returns 403, while a trusted but unauthenticated request returns 401. `dsh web --host 0.0.0.0` remains unsupported, and this login provides no TLS of its own — treat any deployment as loopback- or trusted-network-only. Decision records: [browser request trust](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.md) and [admin-login credential gate](../../../.agents/notes/implemented/feature/2026-09-07-admin-login-credential-gate.md) (which supersedes the earlier [browser token authentication](../../../.agents/notes/implemented/architecture/2026-08-24-browser-token-authentication.md) decision — `dsh web` no longer has a launch-token flow).
