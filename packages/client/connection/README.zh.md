...
每个 Host RPC 方法和 WebSocket stream 都要求同一个浏览器会话，不存在按方法区分的 loopback 层，且获取会话的方式只有一种：用 `ADMIN_EMAIL`/`ADMIN_PASSWORD` 配置的管理员邮箱和密码登录。`requireAdminCredentials`（`src/admin-login.ts`）在插件启动时从进程环境读取这两个变量；只要缺一个，加载就会直接、明确地失败——`dsh web` 没有其他入口，半配置的凭据绝不能悄悄放行。原始密码会在启动时立即用 `scrypt`（配新生成的随机盐）哈希一次后即被丢弃；只有派生出的哈希和邮箱摘要会保留在内存中，且仅限进程生命周期内，绝不持久化或写入日志。

`frontend-static` 把根路径和 index 请求交给 `ctx.connection.authorizeIndex`：未认证的 `GET /` 会返回一个自包含、无脚本的 HTML 登录 `<form method="POST">`（`src/login-form.ts`），而不是纯文本 401；其他任何未认证请求都会得到纯文本 401。该表单提交到 `POST /login`（`src/login-route.ts`），由它验证提交内容——对邮箱和密码都做恒定时间比较（`verifyAdminLogin`），因此邮箱错误与密码错误消耗的 CPU 时间完全相同，也不会暴露到底是哪一项错了——验证通过后签发一个绑定 authority 的签名 cookie。`src/login-throttle.ts` 会在运行密码哈希之前，先对失败尝试按来源施加固定的指数退避（500ms 起倍增，上限 30 秒）；该曲线是安全不变量，不可配置。缺失、过期、畸形或 authority 不匹配的 cookie 会在 RPC 分发前得到 401。静态资源保持公开。HTTP 载体不接受 Authorization header token，也不接受任何形式的 query-string 凭据。

cookie 签名密钥是 `ctx.credentials` 中由 `client-connection/browser-session` 拥有的 grant 记录。本地提供方把它持久化到 `$DSH_HOME/.credentials.yaml`；`BrowserAuth` 在 Connection 激活期间加载或创建该记录，并把密钥留在内存中，因此请求认证同步执行。删除或替换该记录会在下一次 Connection 激活时生效。cookie 携带绝对签发与过期区间，`cookieMaxAgeDays` 默认设为 30 天，并在确定性名称与签名 payload 中同时绑定规范化 hostname 和 port。它是 host-only、`Path=/`、`HttpOnly`、`SameSite=Strict`；随附服务器使用 loopback HTTP，因此刻意不设置 `Secure`。管理员凭据本身不会被持久化到任何地方——它每次启动都通过环境变量重新提供——因此它自身没有撤销状态；删除签名密钥记录并重启会撤销所有现存会话。

认证之前，每个请求仍经过 `src/api-request-trust.ts`。其 `Host` 必须是 loopback，或与 `trustedHosts` 条目匹配：带端口的 `host:port` 精确匹配，不带端口的条目匹配任意端口，两侧均经 WHATWG 归一化。若附带 `Origin`，它必须等于该 Host；`sec-fetch-site: cross-site` 一律拒绝。畸形配置 authority 会让插件加载失败。这些检查防御 DNS rebinding 与跨站浏览器请求，绝不建立身份。Host/Origin 校验失败返回 403；Host 可信但未认证的请求返回 401。`dsh web --host 0.0.0.0` 仍不受支持，且这个登录机制本身不提供 TLS——任何部署都应视为仅限 loopback 或可信网络。决策记录：[浏览器请求信任](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.zh.md)与[管理员登录凭据网关](../../../.agents/notes/implemented/feature/2026-09-07-admin-login-credential-gate.zh.md)（它取代了更早的[浏览器令牌认证](../../../.agents/notes/implemented/architecture/2026-08-24-browser-token-authentication.zh.md)决策——`dsh web` 不再有启动令牌流程）。
