...
import type { AdminCredentials } from './admin-login.ts'
import { verifyAdminLogin } from './admin-login.ts'
import { renderLoginPage } from './login-form.ts'
import type {
...
/**
 * Admin-login submission and persistent signed-cookie verification. This is
 * the only way into `dsh web`: Connection loads the credential provider's
 * signing secret during activation and retains it for synchronous request
 * authentication.
 */
export class BrowserAuth {
   ...
   * @param admin - the configured admin email+password credential.
  ...
  static async create(
    ...
    return new BrowserAuth(await initializeSecret(credentials), maxAgeDays, admin)
  }

  /**
   * The plain application root URL. The operator authenticates by signing in
   * with the configured email and password; there is no credential to carry
   * in the URL itself.
   * @param baseUrl - canonical browser origin.
   * @returns the clean root URL.
   */
  ...
  /**
   * Verify one login submission and mint a session cookie on success.
  ...
  attemptLogin(request: ConnectionTrustRequest, email: string, password: string): string | undefined {
    ...
    if (authority === undefined || !verifyAdminLogin(this.admin, email, password)) return undefined
    return this.mintCookie(authority)
  }

  /** Mint one signed, authority-bound session cookie. */
  private mintCookie(authority: string): string {
   ...
   * index; an unauthenticated `GET /` instead serves the sign-in form; every
   * other unauthenticated request receives the same minimal 401 response.
  ...
  authorizeIndex(req: ConnectionIndexRequest, res: ConnectionIndexResponse): boolean {
    ...
    if (req.method === 'GET' && new URL(req.url ?? '/', 'http://dsh.invalid').pathname === '/') {
